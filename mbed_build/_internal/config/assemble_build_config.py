...
from mbed_build._internal.config.bootloader_overrides import BootloaderOverrides
...
def assemble_config(mbed_target: Target, mbed_program: MbedProgram) -> Config:
    ...
    """
    target_source = Source.from_target(mbed_target)
    mbed_lib_files = find_files("mbed_lib.json", mbed_program.root_path)
...
def assemble_bootloader_overrides(mbed_target: Target, mbed_program_directory: Path) -> Config:
    """Assemble Config for given target and program directory.
    ...
    mbed_app_file = mbed_program.files.app_config_file
    return _assemble_bootloader_overrides_from_sources_and_lib_files(target_source, mbed_lib_files, mbed_app_file)
...
def _assemble_config_from_sources_and_lib_files(
    ...
    while True:
        ...
        mbed_lib_sources = [Source.from_mbed_lib(f, current_cumulative_data.labels) for f in filtered_files]
...
def _assemble_bootloader_overrides_from_sources_and_lib_files(
    target_source: Source, mbed_lib_files: Iterable[Path], mbed_app_file: Optional[Path] = None
) -> Config:
    previous_cumulative_data = None
    current_cumulative_data = CumulativeData.from_sources([target_source])
    ...
    return BootloaderOverrides.from_sources(all_sources)
